...
function sf_render_homepage_admin() {
	...
	<div class="wrap">
		...
		<p><?php esc_html_e( 'برای هر مرحله تصویر، عنوان، توضیح، متن دکمه و لینک را دستی وارد کنید. این اطلاعات در صفحه اصلی نمایش داده می‌شوند.', 'saharfarahani' ); ?></p>
		<form method="post" action="options.php">
			<?php settings_fields( 'sf_homepage_group' ); ?>
			<?php for ( $i = 1; $i <= 6; $i++ ) : $item = $items[ $i ]; $image = ! empty( $item['image'] ) ? wp_get_attachment_image_url( $item['image'], 'thumbnail' ) : ''; ?>
				<div style="background:#fff;border:1px solid #dcdcde;border-radius:12px;padding:20px;margin:0 0 16px;max-width:900px;">
					...
					<div style="display:grid;grid-template-columns:120px 1fr;gap:18px;align-items:start;">
						<div>
							<div class="sf-image-preview" data-preview="<?php echo esc_attr( $i ); ?>" style="width:110px;height:110px;background:#f2f2f2;border-radius:12px;overflow:hidden;margin-bottom:8px;">
							...
							<button type="button" class="button sf-upload-image" data-target="<?php echo esc_attr( $i ); ?>"><?php esc_html_e( 'انتخاب تصویر', 'saharfarahani' ); ?></button>
						</div>
						<div>
							<p><label><strong>عنوان</strong></label><br><input class="regular-text" type="text" name="sf_learning_paths[<?php echo esc_attr( $i ); ?>][title]" value="<?php echo esc_attr( $item['title'] ); ?>" style="width:100%;"></p>
							<p><label><strong>توضیح</strong></label><br><textarea name="sf_learning_paths[<?php echo esc_attr( $i ); ?>][text]" rows="3" style="width:100%;"><?php echo esc_textarea( $item['text'] ); ?></textarea></p>
	...
	document.addEventListener('DOMContentLoaded', function () {
		document.querySelectorAll('.sf-upload-image').forEach(function(button) {
			button.addEventListener('click', function() {
				...
				const frame = wp.media({title: 'انتخاب تصویر', button: {text: 'استفاده از تصویر'}, multiple: false});
				frame.on('select', function() {
					...
					document.querySelectorAll('.sf-image-id')[index - 1].value = attachment.id;
				});
				frame.open();
			});
		});
