...
function sf_setup() {
    ...
    add_theme_support( 'automatic-feed-links' ); add_theme_support( 'responsive-embeds' ); add_theme_support( 'block-template-parts' );
...
function sf_enqueue_assets() {
    ...
    if ( is_singular( 'post' ) ) {
        wp_enqueue_style( 'sf-single-post', SF_URI . '/assets/css/single-post.css', array( 'sf-fixes', 'sf-auth' ), SF_VERSION );
    }
    ...
    if ( is_front_page() ) {
        ...
        wp_localize_script( 'sf-course-latest', 'sfLatestCourses', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce( 'sf_latest_courses_nonce' ), 'count' => max( 1, absint( get_theme_mod( 'sf_latest_course_count', 5 ) ) ) ) );
    }
...
function sf_customizer_css() {
    $primary = get_theme_mod( 'sf_primary_color', '#7c315e' ); $accent = get_theme_mod( 'sf_accent_color', '#c99b5d' ); $dark = get_theme_mod( 'sf_dark_color', '#171417' ); $surface = get_theme_mod( 'sf_surface_color', '#f7f3f0' ); $container = absint( get_theme_mod( 'sf_container_width', 1240 ) );
    ?><style id="sf-customizer-css">:root{--sf-primary:<?php echo esc_attr( $primary ); ?>;--sf-accent:<?php echo esc_attr( $accent ); ?>;--sf-dark:<?php echo esc_attr( $dark ); ?>;--sf-surface:<?php echo esc_attr( $surface ); ?>;--sf-container:<?php echo esc_attr( $container ); ?>px;}</style><?php
}
...
function sf_body_classes( $classes ) {
    ...
    if ( is_singular( 'post' ) ) { $classes[] = 'sf-single-post'; }
...
function sf_register_image_sizes() {
    add_image_size( 'sf-course-card', 720, 480, true ); add_image_size( 'sf-category-card', 720, 560, true ); add_image_size( 'sf-portrait', 720, 900, true );
}
add_action( 'after_setup_theme', 'sf_register_image_sizes' );
function sf_get_course_taxonomy() { return taxonomy_exists( 'course-category' ) ? 'course-category' : 'category'; }
